...
class Phi3Attention(nn.Module):
    ...
    def forward(
        ...
        ### First update, second rotate
        if past_key_value is not None:
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}  # Specific to RoPE models
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)

        cos, sin = self.rotary_emb(value_states, position_ids_k, position_ids_q, seq_len=kv_seq_len)
...
class Phi3SdpaAttention(Phi3Attention):
    ...
    def forward(
        ...
        cos, sin = self.rotary_emb(value_states, position_ids_q)
        query_states = apply_rotary_pos_emb_single(query_states, cos, sin, position_ids_q)

        cos, sin = self.rotary_emb(value_states, position_ids_k)
        key_states = apply_rotary_pos_emb_single(key_states, cos, sin, position_ids_k)
